...
{
    uint32_t pwm_value;
    int8_t abs_duty = abs(duty_cycle);  // 取绝对值作为实际占空比计算基础

    // 限制占空比绝对值最大为100
    if (abs_duty > 100) {
        abs_duty = 100;
    }
    pwm_value = (PWM_PERIOD * abs_duty) / 100;  // 计算PWM值

    if (duty_cycle > 0) {
        // 正值：正转（原MOTOR_FORWARD逻辑）
        setMatch_timer32PWM(1, 1, pwm_value);
        setMatch_timer32PWM(1, 0, 0);
        setMatch_timer16PWM(0, 1, pwm_value);
        setMatch_timer16PWM(0, 0, 0);
    } else if (duty_cycle < 0) {
        // 负值：反转（原MOTOR_BACKWARD逻辑）
